...
func TranslateByTag(dstPtr interface{}) (err error) {
	...
	for i := 0; i < dstPropertyNums; i++ {
		//dstField := elemType.Field(i)
		fieldValue := ememValue.Field(i)
		//logMy.Info("not Ptr==================",dstField)
		// 无效，说明src没有这个属性 || 属性同名但类型不同
		if !fieldValue.IsValid() {
			continue
		}
		//v := reflect.Indirect(srcFieldValue).FieldByName(dstField.Name)
		//if  srcFieldValue.IsNil() || srcFieldValue.IsZero() { //是空或0 不处理
		//	continue //如果是空不处理
		//}
		...
		//logMy.Info("locale:",locale,"====type:",dstField.Type ,"======kind:",reflect.TypeOf(dstField).Kind(),"=======fieldValue:",fieldValue)
		if locale != "auto" {
			continue
		}
		//这里修改 中文key 到它国语言
