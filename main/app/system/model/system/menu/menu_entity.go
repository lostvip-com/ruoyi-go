...
type SysMenu struct {
	MenuId     int64     `json:"menuId" xorm:"not null pk autoincr comment('菜单ID') BIGINT(20)"`
	MenuName   string    `json:"menuName" xorm:"not null comment('菜单名称') VARCHAR(50)"`
	ParentId   int64     `json:"parentId" xorm:"default 0 comment('父菜单ID') BIGINT(20)"`
	...
	MenuType   string    `json:"menuType" xorm:"default '' comment('菜单类型（M目录 C菜单 F按钮）') CHAR(1)"`
...
func (r *SysMenu) Update() (int64, error) {
	return db.GetInstance().Engine().Table(TableName()).ID(r.MenuId).Update(r)
}

// 删除
func (r *SysMenu) Delete() (int64, error) {
	return db.GetInstance().Engine().Table(TableName()).ID(r.MenuId).Delete(r)
}

// 批量删除
func DeleteBatch(ids ...int64) (int64, error) {
	return db.GetInstance().Engine().Table(TableName()).In("menu_id", ids).Delete(new(SysMenu))
}

func DeleteChildren(parentId int64) (int64, error) {
	return db.GetInstance().Engine().Table(TableName()).Where("parent_id", parentId).Delete(new(SysMenu))
}

// 根据结构体中已有的非空数据来获得单条数据
func (r *SysMenu) FindOne() (bool, error) {
	return db.GetInstance().Engine().Table(TableName()).OrderBy("menu_id desc").Get(r)
}
