...
type Entity struct {
	...
	MenuId int64 `json:"menuId" xorm:"not null pk comment('菜单ID') BIGINT(20)"`
}
...
func (r *Entity) Update() (int64, error) {
	return db.GetInstance().Engine().Table(TableName()).ID(core.PK{r.RoleId, r.MenuId}).Update(r)
}

// 删除
func (r *Entity) Delete() (int64, error) {
	return db.GetInstance().Engine().Table(TableName()).ID(core.PK{r.RoleId, r.MenuId}).Delete(r)
}
