...
func (r *SysPost) Update() (int64, error) {
	return db.GetInstance().Engine().Table(TableName()).ID(r.PostId).Update(r)
}

// 删除
func (r *SysPost) Delete() (int64, error) {
	return db.GetInstance().Engine().Table(TableName()).ID(r.PostId).Delete(r)
}

// 批量删除
func DeleteBatch(ids ...int64) (int64, error) {
	return db.GetInstance().Engine().Table(TableName()).In("post_id", ids).Delete(new(SysPost))
}
