...
type Entity struct {
	...
	DeptId int64 `json:"deptId" xorm:"not null pk comment('部门ID') BIGINT(20)"`
}
...
func (r *Entity) Update() (int64, error) {
	return db.GetInstance().Engine().Table(TableName()).ID(core.PK{r.RoleId, r.DeptId}).Update(r)
}

// 删除
func (r *Entity) Delete() (int64, error) {
	return db.GetInstance().Engine().Table(TableName()).ID(core.PK{r.RoleId, r.DeptId}).Delete(r)
}
