...
func New(loginName string, userId, tenantId int64) *MyClaims {
	timeOut := cast.ToInt(conf.Config().GetVipperCfg().Get("jwt.timeout"))

	if timeOut <= 0 {
		timeOut = 3600
	}

	refresh := cast.ToInt(conf.Config().GetVipperCfg().Get("jwt.refresh"))
...
func (claims *MyClaims) CreateToken() (string, error) {
	...
	encryptKey := cast.ToString(conf.Config().GetVipperCfg().Get("jwt.encryptKey"))
