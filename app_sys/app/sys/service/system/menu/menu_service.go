...
import (
	...
	"robvi/app/sys/model"
	"robvi/app/sys/model/system/menu"
...
// 根据主键查询数据
func SelectRecordById(id int64) (*menu.EntityExtend, error) {
	return menu.SelectRecordById(id)
}

// 根据条件查询数据
func SelectListAll(params *menu.SelectPageReq) ([]menu.SysMenu, error) {
	return menu.SelectListAll(params)
}

// 根据条件分页查询数据
func SelectListPage(params *menu.SelectPageReq) (*[]menu.SysMenu, *lv_web.Paging, error) {
	return menu.SelectListPage(params)
}

// 根据主键删除数据
func DeleteRecordById(id int64) bool {
	rs, err := (&menu.SysMenu{MenuId: id}).Delete()
	if err == nil {
...
// 添加数据
func AddSave(req *menu.AddReq, c *gin.Context) (int64, error) {

	var entity menu.SysMenu
...
// 修改数据
func EditSave(req *menu.EditReq, c *gin.Context) (int64, error) {
	entity := &menu.SysMenu{MenuId: req.MenuId}
...
func DeleteRecordByIds(ids string) int64 {
	...
	result, err := menu.DeleteBatch(idarr...)
	if err != nil {
		return 0
	}
	return result
...
// 获取用户的菜单数据
func SelectMenuNormalByUser(userId int64) (*[]menu.EntityExtend, error) {
...
// 获取管理员菜单数据
func SelectMenuNormalAll() (*[]menu.EntityExtend, error) {
	//从数据库中读取
	var result []menu.EntityExtend
	result, err := menu.SelectMenuNormalAll()
...
// 根据用户ID读取菜单数据
func SelectMenusByUserId(userId string) (*[]menu.EntityExtend, error) {
	//从数据库中读取
	var result []menu.EntityExtend
	result, err := menu.SelectMenusByUserId(userId)
...
// 根据父id获取子菜单
func getMenuChildPerms(menus []menu.EntityExtend, parentId int64) []menu.EntityExtend {
	if menus == nil {
		return nil
	}
...
func CheckMenuNameUniqueAll(menuName string, parentId int64) string {
	entity, err := menu.CheckMenuNameUniqueAll(menuName, parentId)
	if err != nil {
		return "1"
	}
...
func CheckPermsUniqueAll(perms string) string {
	entity, err := menu.CheckPermsUniqueAll(perms)
	if err != nil {
		return "1"
	}
	if entity != nil && entity.MenuId > 0 {
...
func RoleMenuTreeData(roleId, userId int64) (*[]model.Ztree, error) {
	...
	if roleId > 0 {
		roleMenuList, err := menu.SelectMenuTree(roleId)
